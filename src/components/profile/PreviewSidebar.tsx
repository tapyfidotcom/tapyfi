...
import { ProfileFormData, BackgroundSettings, getPrimaryColor } from "@/types/profile";
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 lg:gap-3">
                <div className="relative">
                  <Eye size={16} className="lg:w-5 lg:h-5 text-indigo-600 dark:text-indigo-400" />
                  <Sparkles size={8} className="lg:w-3 lg:h-3 absolute -top-1 -right-1 text-purple-500 animate-pulse" />
                </div>
                <div>
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  {isFullscreen ? <Minimize2 size={12} className="lg:w-4 lg:h-4" /> : <Maximize2 size={12} className="lg:w-4 lg:h-4" />}
                </Button>
                <Button 
                  variant="ghost" 
                  size="sm" 
                ...
                >
                  <RefreshCw size={12} className={`lg:w-4 lg:h-4 ${refreshKey > 0 ? 'animate-spin' : ''}`} />
                </Button>
              </div>
            </div>
          </CardHeader>
              ...
              <div className="relative z-10 h-full">
                <div className={`h-full flex flex-col ${
                  ...
                  <div className={`text-center flex-shrink-0 ${
                    ...
                    <div className="flex justify-center mb-4 lg:mb-5">
                      <div className="relative">
                        ...
                        />
                        {/* Subtle ring instead of heavy border */}
                        <div className="absolute inset-0 rounded-full ring-2 ring-white/20 ring-offset-2 ring-offset-transparent pointer-events-none" />
                      </div>
                    </div>
                  ...
                  <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide">
                    <div className="space-y-3 lg:space-y-4 pb-4 lg:pb-5"> {/* INCREASED SPACING */}
                      {links.length > 0 ? (
                        <>
                          {links.slice(0, isFullscreen ? 12 : 8).map((link, index) => { // SHOW MORE LINKS
                            ...
                            return (
                              <button
                                key={link.id}
                                onClick={() => !isLoading && handleLinkClick(link)}
                                disabled={isLoading}
                                ...
                                style={{ 
                                  ...
                                  boxShadow: '0 6px 24px rgba(0, 0, 0, 0.25), inset 0 1px 0 rgba(255, 255, 255, 0.2)', // REDUCED SHADOW
                                  animationDelay: `${index * 50}ms` // FASTER ANIMATION
                                }}
                              >
                                {/* Subtle hover effect */}
                                <div className={`absolute inset-0 bg-gradient-to-r from-white/5 to-transparent opacity-0 transition-opacity duration-200 ${
                                  ...
                                  <div className="flex items-center gap-4 min-w-0 flex-1">
                                    ...
                                    <span 
                                      className={`font-semibold text-white truncate ${
                                        isFullscreen ? 'text-lg' : 'text-base lg:text-lg' // LARGER TEXT
                                      }`}
                          ...
                          {links.length > (isFullscreen ? 12 : 8) && (
                            <div className="text-center py-2">
                              ...
                              >
                                +{links.length - (isFullscreen ? 12 : 8)} more links
                              </Badge>
                            </div>
                          )}
                        </>
                        ...
                        <div className="text-center py-8 lg:py-12">
                          <div className={`mb-3 opacity-50 ${isFullscreen ? 'text-5xl' : 'text-4xl lg:text-5xl'}`}>
                            🔗
                          </div>
                          <p 
              ...
              <div className="p-3 lg:p-4 space-y-3 bg-gradient-to-br from-gray-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900 border-t border-gray-100 dark:border-gray-800">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <TrendingUp size={12} className="lg:w-4 lg:h-4 text-indigo-600" />
                ...
                <div className="grid grid-cols-2 gap-2 lg:gap-3">
                  <div className="bg-white dark:bg-gray-800 rounded-lg lg:rounded-xl p-2 lg:p-3 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-1">
                      <div className="text-xl lg:text-2xl font-bold text-indigo-600">{links.length}</div>
                      <div className="p-1 lg:p-1.5 bg-indigo-100 dark:bg-indigo-900 rounded-md lg:rounded-lg">
                        <ExternalLink size={10} className="lg:w-3 lg:h-3 text-indigo-600 dark:text-indigo-400" />
                      </div>
                    </div>
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-lg lg:rounded-xl p-2 lg:p-3 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="p-1 lg:p-1.5 bg-purple-100 dark:bg-purple-900 rounded-md lg:rounded-lg">
                        <TrendingUp size={10} className="lg:w-3 lg:h-3 text-purple-600 dark:text-purple-400" />
                      </div>
                    </div>
